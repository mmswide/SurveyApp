...
class StripeManaged < Struct.new( :user )
  ...
  def create_account!( country, tos_accepted, ip, legal_params )
    
    return nil unless tos_accepted
    return nil unless country.in?( COUNTRIES.map { |c| c[:code] } )
    byebug
    begin
      # Stripe.api_key = "sk_test_E5lvbnrJeCbFsCnsJwK0bVz7"
      ...
      @account = Stripe::Account.create(
        ...
        legal_entity: legal_params[:legal_entity],
        external_account: legal_params[:bank_account_token]
      )
    rescue => e
      @error = e.message
      nil # TODO: improve
